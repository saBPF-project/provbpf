...
static __always_inline void prov_update_inode(struct inode *inode, union long_prov_elt *prov) {
...
static __always_inline union long_prov_elt* get_or_create_inode_prov(struct inode *inode) {
    union long_prov_elt *prov_tmp;
    uint64_t key = get_key(inode);
    union long_prov_elt *prov_on_map = bpf_map_lookup_elem(&inode_map, &key);
