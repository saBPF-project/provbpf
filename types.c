...
static const char RL_STR_PERM[] = "perm";                                                               // check permission
...
const char *relation_id_to_str(uint64_t type)
{
	switch (type) {
	case RL_READ:
		return RL_STR_READ;
...
const char *node_id_to_str(uint64_t type)
{
	switch (type) {
	case ENT_STR:
		return ND_STR_STR;
